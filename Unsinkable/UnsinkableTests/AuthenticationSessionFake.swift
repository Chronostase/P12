...
class AuthenticationSessionFake: AuthenticationSession {
    let fakeUser: UserDetails
    var fakeStorage: [CustomResponse]
    
    init(fakeUser: UserDetails, fakeStorage: [CustomResponse] ) {
    ...
    override func addUserToDataBase(customResponse: CustomResponse?, _ firstName: String, _ name: String, _ email: String, _ password: String, completion: @escaping (CustomResponse?, Error?) -> Void) {
